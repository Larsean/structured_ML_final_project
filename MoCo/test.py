...
parser.add_argument('--encoder_trn_batch_size', type=int, default=256,
                    help='Batch size of training experiment.')
...
loss_path = os.path.join(config.encoder_output_root, config.encoder_dataset_name, f'bz{config.encoder_trn_batch_size}', config.encoder_exp_version, 'eval/loss')
accr_path = os.path.join(config.encoder_output_root, config.encoder_dataset_name, f'bz{config.encoder_trn_batch_size}', config.encoder_exp_version, 'eval/accr')
weight_path = os.path.join(config.encoder_output_root, config.encoder_dataset_name, f'bz{config.encoder_trn_batch_size}', config.encoder_exp_version, 'eval/weight')
...
ckpt_path = os.path.join(config.encoder_output_root, config.encoder_dataset_name, f'bz{config.encoder_trn_batch_size}', config.encoder_exp_version, 'weight', ckpt_name)
...
while(epoch < config.max_epoch):
    ...
    if (epoch - 1) % config.save_weight_epoch == 0 or epoch == config.max_epoch:
        ...
        with open(os.path.join(loss_path, 'loss_eval.pkl'), 'wb') as f:
            pickle.dump(loss_hist, f)
            
        with open(os.path.join(accr_path, 'accr_eval.pkl'), 'wb') as f:
