...
# TODO: (deprecated) if it doesn't have any dependcy, remove this class.
...
def data_loader(dataset_root='placeholder, use .stl10_dest_pth. variable', resize=84, crop=64, 
                batch_size=64, num_workers=16, type='train'):    
    '''
    Data loader for MoCo. It is written assuming that 'stl10' dataset is used to train an encoder in 
    ...
    if type == 'encoder_train':
        ...
        dset = Datasets.STL10(root=stl10_dest_pth, split='unlabeled', transform=ContrastiveLearningViewGenerator(transform), download=False)
    elif type == 'classifier_train' or type == 'classifier_test':
        split = type.split('_')[-1] # 'train' or 'test'
        dset = Datasets.STL10(root=stl10_dest_pth, split=split, transform=transform, download=False)
        
    dlen = len(dset)
    dloader = DataLoader(dataset=dset, batch_size=batch_size, shuffle=True, num_workers=0) # num_workers=0 is used to avoid a bug in windows system.
    return dloader, dlen